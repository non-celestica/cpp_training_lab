 ...
 */
std::vector < Unit* > createSquad(int knightCnt=0, int fighterCnt=0, int mageCnt=0, int wizzardCnt=0)
{
    ...
    for (int i=0; i<mageCnt; i++) {
        Mage *m = new Mage();
        std::string name = "Mage#"+std::to_string(i+1);
        m->setMageStats(name, 30, 50);
        squad.push_back(m);
    }

    for (int i=0; i<wizzardCnt; i++) {
        Wizzard *w = new Wizzard();
        std::string name = "Wizzard#"+std::to_string(i+1);
        w->setMageStats(name, 50, 150);
        squad.push_back(w);
    }
    return squad;
}
...
void squadInfo(std::vector < Unit* > &squad)
{
    std::map < UnitType, int > typeCntMap;
    for (auto it=squad.begin(); it!=squad.end(); ++it) {
        typeCntMap[(*it)->getType()]++;
    }

    printf("There are %d squad members", squad.size());
    printf(" knight: %d", typeCntMap[UNIT_TYPE_KNIGHT]);
    printf(" fighter: %d", typeCntMap[UNIT_TYPE_FIGHTER]);
    printf(" mage: %d", typeCntMap[UNIT_TYPE_MAGE]);
    printf(" wizzard: %d", typeCntMap[UNIT_TYPE_WIZZARD]);
}
...
int main() 
{
    ...
    std::vector < Unit* > squad2 = createSquad(2, 1, 0, 2);
    disbandSquad(squad2);
     ...
     */
    std::vector < Unit* > squad3 = createSquad(3, 10, 2, 4);
    squadInfo(squad3);
    disbandSquad(squad3);
